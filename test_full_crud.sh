...
# POB Management System - Full CRUD Test Script
...
if echo "$RESPONSE" | grep -q '"success":true'; then
    ...
    echo -e "${RED}✗ Vessel retrieval failed${NC}"
fi
...
if echo "$RESPONSE" | grep -q '"success":true'; then
    ...
    echo -e "${RED}✗ Vessel update failed${NC}"
fi
...
if [ -n "$ROLE_ID" ]; then
    ...
    echo -e "${YELLOW}⚠ Role creation may have failed${NC}"
fi
...
if echo "$RESPONSE" | grep -q '"success":true'; then
    ...
    echo -e "${RED}✗ Personnel update failed${NC}"
fi
...
if [ -n "$ROLE_ID" ]; then
    ...
    if echo "$RESPONSE" | grep -q '"success":true'; then
        ...
        echo -e "${RED}✗ Rotation schedule creation failed${NC}"
    fi
...
if [ -n "$TRANSPORT_ID" ]; then
    ...
    echo -e "${RED}✗ Transport creation failed${NC}"
fi
...
if [ -n "$SCHEDULE_ID" ]; then
    ...
    echo -e "${RED}✗ Travel schedule creation failed${NC}"
fi
...
if [ -n "$SCHEDULE_ID" ]; then
    ...
    RESPONSE=$(call_api "DELETE" "/travel/schedules/$SCHEDULE_ID")
...
if [ -n "$ROOM_ID" ]; then
    echo "Deleting room..."
    RESPONSE=$(call_api "DELETE" "/rooms/$ROOM_ID")
    if echo "$RESPONSE" | grep -q '"success":true'; then
